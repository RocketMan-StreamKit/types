...
Перед тим як StreamKit+ уб’є worker аддона, головний процес викликає вбудовану подію `addon:prepare-stop` і чекає завершення обробника (короткий таймаут, близько **5 секунд**). Використовуйте її для віддаленого cleanup, поки ще доступні OAuth-токен і мережа.
...
| Право | Не потрібне |
| Таймаут | ~5 с — тримайте роботу короткою; незавершене буде перервано, worker уб’ють |
