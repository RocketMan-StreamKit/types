...
| API | Примітки |
| --- | --- |
| `setTimeout(fn, delay, ...args)` | Помилки перехоплюються; delay передається в Node як є |
| `setInterval(fn, interval, ...args)` | Помилки перехоплюються; interval передається в Node як є |
