...
Outbound HTTP (`network.request`), WebSocket (`network.websocket`), Server-Sent Events (`network.sse`), and SignalR (`network.signalr`) URLs are validated:
...
- SSE: max **5** concurrent connections, `http://` / `https://` only, `text/event-stream`, redirects **not** followed, optional `autoReconnect` (honors SSE `retry:`), same host blocklist
...
Uncaught errors outside those wrappers can still crash the worker process. After **3 consecutive crashes** without a stable run, auto-restart stops until the user restarts the addon from settings (or restarts the app) — see [Lifecycle — Crash loop protection](./lifecycle.md#crash-loop-protection).
