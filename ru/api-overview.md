...
| `alerts` | Состояние показа оповещений между аддонами: `started` / `ended` / `isActive` / `onChange` (без разрешения) |
...
| `tts` | Озвучка текста и информация о голосе (разрешение `TTS`) |
...
| `setTimeout` / `setInterval` / `clearTimeout` / `clearInterval` | Таймеры с изоляцией ошибок |
...
- [Оповещения (`alerts`)](./api-alerts.md)
