...
Чтобы определить язык **текста сообщения** (не локаль UI), используйте [`language.detect`](./api-language.md) — возвращает ISO-коды из fastText lid.176 (~176 языков) и не зависит от `LANG`.
