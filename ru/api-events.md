...
| --- | --- | --- |
| `addon:prepare-stop` | Перед убийством воркера (отключение, удаление или выход из приложения) | `{}` — завершите удалённую очистку; держите обработчик коротким (~10 с таймаут). См. [Жизненный цикл — Корректная остановка](./lifecycle.md#корректная-остановка--addonprepare-stop) |
