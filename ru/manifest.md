...
| Поле | Тип | Описание |
| --- | --- | --- |
| `id` | `string` | Стабильный идентификатор аддона (URL, `depends_on`, записи об установке) |
...
`id` должен быть уникальным среди установленных аддонов. Используется в URL (`/addon/{id}/…`, `/addon_static/{id}/…`) и не обязан совпадать с именем папки.
