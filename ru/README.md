...
| [Жизненный цикл](./lifecycle.md) | Загрузка и выполнение аддона |
| [manifest.json](./manifest.md) | Поля манифеста, типы, правила валидации |
| [Публикация и релизы](./publishing.md) | Структура GitHub-релиза и ID, привязанный к репозиторию |
...
После изменений API песочницы перегенерируйте объявления:

```bash
npm run type:addons
```

Результат: `data/addon.d.ts` (`declare global` — используйте в TypeScript аддона без импортов).
